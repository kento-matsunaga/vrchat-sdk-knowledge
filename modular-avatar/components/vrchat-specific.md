...
## MA Rename VRChat Collision Tags {#rename-collision-tags}

公式: https://modular-avatar.nadena.dev/docs/reference/rename-collision-tags  
追加バージョン: v1.13.0

ContactsのCollision Tagを名前空間化して他Prefab・他アバターとの衝突を防止する。  
MA Parametersと同様に、衝突タグ名を自動的にユニーク名へとリネームする。

### 問題と解決

```
問題: 複数の衣装Prefabが同じCollision Tag名（例: "hand_pat"）を使っている
     → 互いのContactが誤検出される

解決: MA Rename VRChat Collision Tags を追加すると、
     ビルド時にタグ名を一意な内部名に自動変換（他Prefabと衝突しない）
```
...
```
衣装Prefab
  [MA Rename VRChat Collision Tags] ← Prefabルートに追加するだけ
  Contact_Sender/
  Contact_Receiver/
    ← 内部のCollision Tagが自動リネームされる
```

### 特徴

- 配布用Prefabに推奨される（エンドユーザーは意識不要）
- MA Parametersのパラメータ名自動リネームと同じ思想

---

## MA Move Independently {#move-independently}

公式: https://modular-avatar.nadena.dev/docs/reference/move-independently  
動作: **エディタのみ**（ランタイム・ビルド後には影響しない）

子オブジェクトの位置を変えずに、親オブジェクトだけを移動できる衣装調整ツール。

### 使い方

```
問題: Hipボーンを動かすと子ボーン（UpperLeg等）も一緒に動いてしまう
     → 腰の位置だけを微調整したい

解決: HipにMA Move Independentlyを追加
     → Hipを動かしてもUpperLeg/LowerLegは元の位置を維持
```
...
| パラメータ | 説明 |
|---------|------|
| Objects to move together | 一緒に動かしたいオブジェクトのリスト |

### 注意事項

- 非均等スケーリング（X/Y/Z の値が異なるスケール）は完全サポート外
- スケール調整が必要な場合は `MA Scale Adjuster` と併用する
